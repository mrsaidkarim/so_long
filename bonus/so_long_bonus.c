...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/15 14:56:18 by skarim            #+#    #+#             */
/*   Updated: 2024/01/19 15:33:53 by skarim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	ft_init_ghost(&my_game);
	mlx_hook(my_game.win_ptr, 2, 0, ft_move_player, &my_game);
	mlx_hook(my_game.win_ptr, 17, 0, ft_destroy_notify, &my_game);
	mlx_string_put(my_game.mlx_ptr, my_game.win_ptr, 5, 10,
		0xFF0000, "Moves:0");
	mlx_loop_hook(my_game.mlx_ptr, ghost_animation, &my_game);
